...
<body>
    <form action="cdmx.php" method="post">
    <?php
    $con = mysqli_connect("localhost","root","Lasric.2018","Minmer");
    $tem_id = round($_GET['ids']);
    $sqlFC = "SELECT FechaC FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultFC = mysqli_query($con,$sqlFC);
    $sqlFE = "SELECT FechaE FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultFE = mysqli_query($con,$sqlFE);
    $sqlOper = "SELECT Operador FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultOper = mysqli_query($con,$sqlOper);
    $sqlPlac = "SELECT Placas FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultPlac = mysqli_query($con,$sqlPlac);
    $sqlID = "SELECT ID FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultID = mysqli_query($con,$sqlID);
    $sqlOS = "SELECT SO FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultOS = mysqli_query($con,$sqlOS);
    $sqlFact = "SELECT Factura FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultFact = mysqli_query($con,$sqlFact);
    $sqlCli = "SELECT Cliente FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultCli = mysqli_query($con,$sqlCli);
    $sqlPZS = "SELECT PZS FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultPZS = mysqli_query($con,$sqlPZS);
    $sqlCaja = "SELECT Caja FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultCaja = mysqli_query($con,$sqlCaja);
    $sqlSub =  "SELECT Subtotal FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultSub = mysqli_query($con,$sqlSub);
    $sqlHor = "SELECT Horario FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultHor = mysqli_query($con,$sqlHor);
    $sqlDire = "SELECT Direccion FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultDire = mysqli_query($con,$sqlDire);
    $sqlDest =  "SELECT Destino FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultDest = mysqli_query($con,$sqlDest);
    $sqlConce = "SELECT Concepto FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultConce = mysqli_query($con,$sqlConce);
    $sqlCapa = "SELECT Capacidad FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultCapa = mysqli_query($con,$sqlCapa);
    $sqlObser = "SELECT Observaciones FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultObser =  mysqli_query($con,$sqlObser);
    $sqlOE = "SELECT OE FROM CDMX WHERE ID_SQL=".$tem_id.";";
    $resultOE = mysqli_query($con,$sqlOE);
    $sqlCust = "SELECT Custodia FROM CDMX WHERE ID_SQL=".$tem_id.";";
